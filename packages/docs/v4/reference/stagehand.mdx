...
<ParamField path="schema" type="StagehandSchema | StagehandClientExtractOptions" optional>
  ...
  output. Validate-only schemas and plain JSON Schema documents are not accepted directly; wrap
  them with `jsonSchema(...)`. The helper preserves the complete Draft 2020-12 document.
  Zod 4.2.0+ and ArkType work directly. Wrap Valibot schemas with the official
...
<ResponseField name="result" type="Promise<ExtractResult<Schema>>">
  ...
  <ResponseField name="result.data" type="Schema">
    Data validated against the caller's schema. Pass `ExtractResult<typeof schema>` to name the
    result; `data` is the schema's output type.
  </ResponseField>
...
| TypeBox | Pass the complete schema to `jsonSchema<T>()` | TypeBox 1.3.7 tested |
...
the complete schema to `jsonSchema()`. The explicit generic preserves TypeBox's static type,
...
Pass a complete Draft 2020-12 document. `jsonSchema()` preserves its required and optional
properties, definitions, references, and additional-property policy. `JsonSchemaDocument` is a
JSON object type. The hardened profile is what `jsonSchema()` actually accepts.
...
Do not pass a TypeBox object or plain JSON Schema document directly to `extract()`. Wrap the full
document with `jsonSchema()` first.
