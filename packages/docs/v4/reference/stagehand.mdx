...
JSON object type. The extension hardens Draft 2020-12 after the schema crosses RPC.
...
Before the browser call, Stagehand asks the schema for its Draft 2020-12 input representation.
The extension then hardens that document: it rejects unsupported vocabularies, external references,
nested identifier scopes, malformed keywords, and schemas that exceed resource limits.
