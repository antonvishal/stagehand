...
<ParamField path="schema" type="StagehandSchema | StagehandClientExtractOptions" optional>
  ...
  output. Validate-only schemas, plain JSON Schema, and TypeBox schemas are not accepted directly.
...
Minimum supported versions are Zod `4.2.0`, ArkType `2.1.28`, Valibot `1.2.0`, and
`@valibot/to-json-schema` `1.5.0`. The workspace also tests Zod `4.4.3`, ArkType `2.2.3`, Valibot
`1.4.2`, and adapter `1.7.1`.
...
<Tab title="Custom">
```typescript
import type { StagehandSchema } from "@browserbasehq/stagehand";

const schema: StagehandSchema<string, number> = {
  "~standard": {
    version: 1,
    vendor: "my-lib",
    types: undefined,
...
const { data } = await stagehand.extract("extract label", schema);
```
</Tab>
</Tabs>
